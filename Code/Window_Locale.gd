...
func _ready():
	...
	exe_path = $"..".exe_path;
	locale_folder = exe_path+"/"+locale_folder_name;
...
func save_locale(locale_name):
	if directory.dir_exists(locale_folder) == false:
		if directory.make_dir(locale_folder) == OK: $"%base_actions_hint2".text = "good";
		else: $"%base_actions_hint2".text = "bad";
	else: $"%base_actions_hint2".text = "dir already present";
	if directory.dir_exists(locale_fonts_folder) == false:
		if directory.make_dir(locale_fonts_folder) == OK: $"%base_actions_hint2".text = "good2";
		else: $"%base_actions_hint2".text = "bad2";
